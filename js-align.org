...
should fail on a comment which also uses an ~*~ in either first or
second character position. Perhaps remove the ~*~ from the group of
characters to be excluded in second character position, and create
